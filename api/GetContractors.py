import requests

class EnovaApp:
    def __init__(self):
        # Podstawowy token generowany przez Enova365 (przykładowy)
        self.base_token = "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..k4SbzPbPheoNn7CajBhWj0CzECeb5HzIEvSzahyeC3TaPiIcdxbnlijKZp2loe6JXu1z62V2pS4erLMpOBUUFrSnbet36iZpcXWGbL69VI3GCSHnwRcD1ssMYMxp6pKTZf0OdRfKlfyFeI4ntZdzh2vs7aBFhaHE0m5Wv3EpH3Biv9dVMACu6ayAthg4Of6vSMRfr7T_MZDN35aCYMsWordxlWUJwJ5K3YrpjfLAQXNLEsjfkv1dcqHY3NZtj2c1Z84PApRpIeo1kjJLvV5-7UIt7xX1cPMuC3omZfhEmUAuYe31YMHiLhb7QThx9Zcd33Tq0fYWPLe-pzVLelrthg"

    def send_request(self, param):
        login_url = "http://192.168.0.23:6001/api/LoginApi"
        service_url = "http://192.168.0.23:6001/api/ServiceImpApiANS/GetKontrahenci"  # Zastąp odpowiednim endpointem

        headers = {
            'Authorization': f'Bearer {self.base_token}',
            'Content-Type': 'application/json'
        }

        try:
            # Login and get the session token
            response = requests.post(login_url, headers=headers)
            response.raise_for_status()
            login_data = response.json()
            session_token = login_data.get('Token')
            print(f"Response: {response}")
            print(f"Login Data: {login_data}")
            print(f"Session Token: {session_token}")

            if not session_token:
                raise ValueError("Nie udało się uzyskać tokenu sesji")

            # Now use the session token to access the service
            service_headers = {
                'Authorization': f'Bearer {session_token}',
                'Content-Type': 'application/json'
            }
            service_payload = {
                'param': param  # Dodaj inne wymagane parametry do payload
            }
            service_response = requests.post(service_url, headers=service_headers, json=service_payload)
            service_response.raise_for_status()
            data = service_response.json()
            print(f"Wynik: {data}")
        except requests.exceptions.RequestException as e:
            print(f"Błąd podczas komunikacji z API: {e}")
        except ValueError as ve:
            print(f"Błąd: {ve}")

if __name__ == "__main__":
    app = EnovaApp()
    param = ""
    app.send_request(param)
